...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      // OPTIMIZATION: Polling disabled in favor of webhooks
...
// Webhook endpoint for scraper status updates
router.post('/:runId/status', async (req, res) => {
  try {
    const { runId } = req.params;
    const { status, result, error } = req.body;

    console.log(`📥 Received status update for run ${runId}: ${status}`);
    ...
    if (status === 'success') {
      ...
      try {
        emitRunCompleted(run.userId, runId, run.toObject());

        // Notify user
        await notifyRunCompleted(run.userId, {
          runId: run.runId,
          actorName: run.actorName,
          actorId: run.actorId,
          resultCount: run.resultCount
      ...
      // Trigger background enrichment for Google Maps if needed
      if (run.actorId === 'google-maps' && Array.isArray(result.data) && result.data.length > 0) {
        try {
          console.log(`🔍 Triggering background enrichment for ${result.data.length} places in run ${runId}`);
          await axios.post(`${SCRAPER_SERVICE_URL}/enrich`, {
            run_id: runId,
      ...
      run.status = 'failed';
      run.error = error || 'Scraping failed';
      run.finishedAt = new Date();
      ...
      // Emit failure event
      try {
        emitRunFailed(run.userId, runId, run.error);

        // Notify user
        await notifyRunFailed(run.userId, {
          runId: run.runId,
          actorName: run.actorName,
          actorId: run.actorId,
          error: run.error
      ...
      if (run.status !== 'running') {
        run.status = 'running';
        run.startedAt = new Date(); // Update start time
        await run.save();

        try {
          emitRunStatusChange(runId, 'running', { userId: run.userId });
          emitRunUpdate(run.userId, run.toObject());
          // Notify user
          await notifyRunStarted(run.userId, {
            runId: run.runId,
            actorName: run.actorName,
            actorId: run.actorId
          });
